...
public class ToylanguageModelStateHelper implements IK3ModelStateHelper{
	...
	public K3ModelState getK3ModelState(EObject model) {
		...
		while (allContentIt.hasNext()) {
			...
			Class<?> clazz =null;
			clazz = K3DslHelper.getTarget(fr.unice.polytech.si5.toylanguage.rewritingrules.VariableAspect.class);
			if (clazz.isInstance(elem)) {
				ElementState elemState = theFactory.createElementState();
				elemState.setModelElement(elem);
				res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("currentValue", ToylanguageRTDAccessor.getcurrentValue(elem));
	...
	public void restoreModelState(K3ModelState state) {
		for(ElementState elemState : state.getOwnedElementstates()) {
			for(Object o : elemState.getSavedRTDs()) {
				...
				try {
					if (n2v.value != null) {
						Method m = ToylanguageRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass());
					...
					for(Class<?> c : n2v.value.getClass().getInterfaces()) {
						...
						try {
							m = ToylanguageRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass().getInterfaces()[0]);
							m.invoke(null, elemState.getModelElement(), n2v.value);
